"""Shared plumbing for the harness: paths, subprocess, JSON, hashing, logging.

Deliberately dependency-free (stdlib only) so `detect_release.py` and
`verify.py` can run on a bare runner before anything is installed.
"""
...
# can keep several runs side by side.
WORK_DIR = Path(os.environ.get("HARNESS_WORK_DIR", REPO_ROOT / ".harness-work"))
EVIDENCE_DIR = Path(os.environ.get("HARNESS_EVIDENCE_DIR", WORK_DIR / "evidence"))

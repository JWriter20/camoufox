    ...
    if block_webgl or launch_options.pop('allow_webgl', True) is False:
        ...
        if webgl_config:
            ...
            try:
                ...
                # The preset's GPU is not in the WebGL catalogue (roughly 10% of
                # the bundled presets), so there are no parameters recorded for
                # it. Leaving the vendor/renderer strings in place and merging
                # another GPU's parameters around them is worse than the
                # substitution -- the reported extensions, limits and shader
                # precisions would contradict the name. Drop the pair so the
                # sampled one below is used for the whole WebGL identity.
                LeakWarning.warn('preset_webgl_unavailable', i_know_what_im_doing)
                del config['webGl:vendor']
                del config['webGl:renderer']

...
from .captcha import configure as configure_captcha
...
def NewBrowser(
    ...
    *,
    ...
    captcha: Optional[Union[bool, str, Dict[str, Any]]] = None,
    **kwargs,
    ...
    Parameters:
        ...
        captcha:
            ...
            `solve_captcha(page)` knows where to send work. A token uses the
            hosted service; a URL uses your own server; optional `model`
            picks which served adapter to ask for:
                captcha="ck_live_..."                    # hosted
                captcha={"url": "http://host:8000/v1"}   # self-hosted
                captcha={"url": "...", "model": "captcha-v12"}
                captcha=True                             # use the environment
            Consumed here rather than passed on: it configures the solver in
            THIS process, and has nothing to do with how the browser launches.
        **kwargs:
            All other keyword arugments passed to `launch_options()`.
    """
    if captcha is not None:
        configure_captcha(captcha)

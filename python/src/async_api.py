...
async def AsyncNewBrowser(
    ...
    **kwargs,
    ...
    Parameters:
        ...
        captcha:
            Credentials for the optional CaptchaKraken solver — see
            `NewBrowser`. Accepted here so `verify_credentials()` and a solve
            offloaded to a worker thread are configured the same way, but NOTE
            that `solve_captcha` itself is SYNCHRONOUS ONLY: a sync Playwright
            handle cannot be driven from inside an event loop, so it cannot be
            called on a page from `AsyncCamoufox`. The TypeScript twin has no
            such limit — its driver is async.
        **kwargs:

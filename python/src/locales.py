...
def normalize_locale(locale: str) -> Locale:
    ...
    # Use the EXPLICIT script from the tag, never the language's suppress-script.
    #
    # Suppress-Script is the implicit default ("Latn" for "en") that BCP-47 says
    # to omit. Emitting it sets `locale:script`, which the browser applies to
    # every mozilla::intl::Locale, so "en-US" is built as "en-Latn-US". ICU's
    # available-locale set has "en-US" but not "en-Latn-US", so Intl falls back
    # to plain "en" while navigator.language stays "en-US" -- a detectable
    # mismatch. Verified against the browser: with the suppress-script emitted,
    # Intl.DateTimeFormat().resolvedOptions().locale reports "en" instead of
    # "en-US". Reading the explicit subtag also preserves genuinely
    # script-bearing locales ("zh-Hans-CN", "sr-Latn-RS"), which suppress-script
    # dropped.

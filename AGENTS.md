...
Search for an existing implementation first: `pythonlib/camoufox/`,
`typescript/src/`, `ci/`, `scripts/` and `additions/`. If something close
...
`pythonlib/` is the reference implementation. `typescript/` ports it bit for
bit, and its golden fixtures fail on any drift. A behaviour change in one lands
...
- **No flakes.** An intermittent failure means something is non-deterministic.
  ...
  green.
- **A missing prerequisite fails in CI; it never passes as a skip.** Use
  `typescript/tests/prereq.ts`, or the equivalent guard in Python.
...
| `typescript/` | The npm package, a bit-exact port of `pythonlib/`. |
...
| `typescript/` | `python3 -m ci.run_typescript`, or `pnpm test` in `typescript/` |

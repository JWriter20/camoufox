...
driven identically from three places: a pull request, a push to main, and the
Firefox auto-update harness. That is deliberate -- the harness proves a Firefox
bump is safe by running the same checks a contributor's pull request runs, so
there is exactly one definition of "the tests pass".
